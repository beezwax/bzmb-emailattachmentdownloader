...
const getMessagesWithAttachments = async (config) => {
  ...
  try {
    const messageGenerator = client.fetch({seen: false}, { envelope: true, source: true, bodyStructure: true });
    ...
    for (const childNode of childNodes) {
      ...
      message.attachments.push({filename: meta.filename, data: base64Content});
    }
